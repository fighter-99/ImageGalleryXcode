...
struct ImageImporterErrorTests {
    ...
    @Test func importURLsOnUnsupportedFormatSkipsNotFails() async throws {
        ...
        let result = await importer.importURLs([txtURL])
    ...
    @Test func importURLsOnNonExistentSourceReturnsFailure() async throws {
        // 文件不存在 → PhotoStorage.importFile 抛错 → failures 收集
        let context = try makeContainer()
        let importer = makeImporter(in: context)
        let badURL = URL(fileURLWithPath: "/tmp/noexist_\(UUID().uuidString).jpg")
        let result = await importer.importURLs([badURL])
        #expect(result.inserted == 0)
    ...
    @Test func importResultFailureCountReflectsMultipleFailures() async throws {
        // 多个不存在文件 → 多个 failures
        let context = try makeContainer()
        let importer = makeImporter(in: context)
        let bad1 = URL(fileURLWithPath: "/tmp/no1_\(UUID().uuidString).jpg")
        let bad2 = URL(fileURLWithPath: "/tmp/no2_\(UUID().uuidString).jpg")
        let result = await importer.importURLs([bad1, bad2])
