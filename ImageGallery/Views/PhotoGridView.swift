...
struct PhotoGridView: View {
    ...
    // ─── 批量移动到文件夹（V3.5 Phase 2：支持撤销）───
    private func batchMove(to folder: Folder?) {
        let photosToMove = photos.filter { selectedIDs.contains($0.id) }
    ...
    private func batchAddTag(_ tag: Tag) {
        let photosToTag = photos.filter { selectedIDs.contains($0.id) }
    ...
    private func batchExport() {
        let photosToExport = photos.filter { selectedIDs.contains($0.id) }
        ...
        for photo in photosToExport {
            ...
            do {
                if FileManager.default.fileExists(atPath: destURL.path) {
                    // 重名加 UUID 后缀
                    let uniqueDest = uniqueDestination(for: destURL)
    ...
    private func batchToggleFavorite() {
        let photosToToggle = photos.filter { selectedIDs.contains($0.id) }
