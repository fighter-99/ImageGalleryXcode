...
struct SidebarView: View {
    ...
    // V6.23 (Bug fix): smart folders 独立 section 持久化 — 跟 library/folders/tags 平级
    @AppStorage("sidebar.section.smartFolders") private var isSmartFoldersExpanded: Bool = true
    ...
    var body: some View {
        ...
        // V6.116: 删 V6.115 加的 .overlay(alignment: .trailing) { Divider() } — 改回 NavigationSplitView
        //   NS 系统自动渲染 sidebar 右边框, 不需要手画 Divider
        sidebarContent
            .background(.bar)
    }
...
// V6.116: 删 SidebarStatusBar struct — 改回 NavigationSplitView 2-col
//   NS 系统自动渲染 sidebar status bar (toolbar .principal 段), 不用手画
//   V6.115 加的 SidebarStatusBar 是 V6.113 漏 NS UI 的妥协方案, V6.116 完全 revert
